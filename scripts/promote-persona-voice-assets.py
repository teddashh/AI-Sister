...
def main() -> None:
    ...
    try:
        ...
        (staging / "NOTICE.md").write_text(
            ...
            "about 120 ms of natural lead-in or run-out, and every trimmed clip was "
            "re-transcribed to confirm the cut had taken none of the written line "
            "with it: no character the same transcriber heard before the cut could "
            "be missing after it. That is a before-and-after comparison; these "
            "transcribers also mishear characters on clips that were never trimmed, "
            "so it is not a claim that any transcript matches the line exactly. "
            "Clips that trimming could not fix were re-recorded instead: two "
            "speech recognition models trained on different data both transcribed "
            "syllables the written line does not contain, and the silence trim above "
            "had already run on those files without removing them. Each was "
            "re-synthesised until the two models no longer agreed on any syllable "
            "beyond the written line, and the new take had to pass the same loudness, "
            "true-peak and speaker-identity checks as the one it replaced. No "
            "compression, limiting, or other dynamics processing was applied.\n\n"
            "This directory holds only the Ogg files, `manifest.json` with its "
            "`manifest.js` mirror, and this notice. The WAV masters, the reference "
            "recordings the voices were conditioned on, and the quality-control "
            "receipts stay in the voice lab and are not published.\n\n"
