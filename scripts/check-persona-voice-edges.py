...
- **編碼器自己造出來的空白。** 現在剩下的 5 支裡有 4 支是這種——kimi「午安……」的
  ...
  alpha.139 重錄過的 hunyuan/tap-quiet 就是這樣進榜的：WAV 兩端都是 0 毫秒，
  出貨的 Ogg 尾巴 580 毫秒。四支的 WAV／Ogg 實測是 20→350、60→960、0→330、
  0→580；剩下那一支 mimo/poke-annoying 是 400→420，那一段在錄音裡就真的存在。
"""
...
# 960 毫秒（kimi「午安……」的結尾，成因是 Opus 把很小聲的東西壓得更小聲，見上面
# 「擋不住的」）；alpha.137 是 1350 毫秒。
HARD_MS = 1200
