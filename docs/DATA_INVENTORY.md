...
> 和上一段同一個：它們是容器上的標籤。
>
> **alpha.33 以前就已經被清空的資料庫拿不到這個位元**，因為升級那一刻現貨是
> 空的，而「它到底存過沒有」在那個檔案裡沒有任何一個位元答得出來。那種資料庫
> 會讀成「沒存過」。這是刻意選的方向：把清空過的說成「沒存過」，你知道自己刪
> 過，那句話只是多餘；把沒存過的說成「被忘掉了」，是指控。
>
> **而「沒存過」自己又是兩種。** 一台跑完收工、一列都沒落地的機器（去看
> `capture.enabled`），和一台**此刻正開著、還沒落地第一列**的機器（再等一
> 下）——下一步是相反的。分它們的不是資料庫裡的位元，是 `recording.beat` 那個
> 心跳檔：`heartbeat::is_occupied`。所以問「這是哪一種空」的那幾頁都要拿著
> `data_dir`，光有 `Db` 答不出來。
>
