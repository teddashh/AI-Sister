...
同意書、而且接好一支大腦 CLI 之後，才會交給那支你自己已經在跑的 CLI。S1 每次只送
當前問題與本機 facts／FTS 已選出的最多 12 筆來源文字及其時間、app、title、URL metadata；
問題副本最多 2 KiB，圍欄內資料合計最多 12 KiB。沒簽、沒設定或本機沒有候選，一次都不
為這題 spawn。外送紀錄只記結構和計數，不抄問題、來源或 prompt 原文。Azure TTS 不取得
...
| 把螢幕上的文字原文（OCR 抽出來的字，永不含畫面）交給你設定的本機 CLI | L2/L3 解讀與 S1 問答成句都不會呼叫那支 CLI |
...
- **第三張是降級，不是拒絕。** 沒簽她照常記錄，只是設定裡的 `store_images` 會
  被當場關掉——那一次錄製一張截圖都不寫
- **第二張是出境閘門。** 沒簽，L2/L3 解讀與 S1 問答成句一次都不會 spawn 那支 CLI。而且這扇門
...
`sister consent` 和使用者第一次開啟字母人時跳出來的那一頁都從 core 取同一份條文與
