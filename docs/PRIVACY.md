...
- **留下來的只有一個位元：她曾經錄過。** 沒有時間、沒有長度、沒有版本，重建不出
  任何東西——而少了它，`sister stats`、`sister doctor` 和字母人那一頁會在你刪完
  的下一秒說「還沒錄過，先去按開始記錄」，也就是叫你重做一件你剛剛才刻意做掉的
  事。那一頁現在會說「她錄過，那些東西被忘掉了或過了保留期」，講的是**現在這顆
  資料庫**，不是那幾天。想連這個位元都不要，就整個刪掉 `sister.db`
...
**這是整份文件裡唯一一個由你把資料搬出去的地方，所以講清楚：匯出檔沒有加密。**
