...
她自己的視窗是唯一的例外。每一拍抓圖前後，她各列一次畫面上的視窗，她看得見的
那幾塊在算 dhash、跑 OCR、存圖之前就塗成黑的；別的程式的視窗蓋在她上面、而且
確定不透明的那一塊，才照原樣留下。塗的是她整扇視窗的範圍：桌寵四周透明、看得到
底下程式的地方也一起塗，被她蓋住的那一小塊那一拍不記。她的一扇視窗開著卻讀不到
位置時，那一拍整張不抓。

Linux X11 預覽版也一樣塗。哪一扇是她的，問的是 X server——它從連線認得出是哪個
程式開的——視窗管理員替她畫的框和標題列一起塗。32 位元色深、`_NET_WM_WINDOW_OPACITY`
設成半透明、形狀不是方的視窗，都不拿來擋她。列視窗的途中有一扇剛好關掉、問不出來，
那一拍整張不抓。合成管理員自己把視窗畫成半透明的設定從視窗上讀不到：那種視窗蓋在
她上面時，底下透出來的她不會被塗。
