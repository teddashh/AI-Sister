...
- **和保留期不一樣，這裡不留字。** 保留期是兩段式的（先丟圖、字留著），因為
  ...
  全部一起消失
- **她那段時間動過的手也一起走。** `action-log.jsonl`（alpha.69 起，只有你按過
  「要我幫你打開嗎」那顆按鈕才會有）是資料庫**旁邊的另一個檔案**，裡面有完整
  的網址和檔案路徑。忘掉一段時間會把落在那段裡的列從檔案裡刪掉——刪的是那串
...
- **留下來的通常只有一個位元：她曾經錄過。** 沒有時間、沒有長度、沒有版本，
  ...
  **現在這顆資料庫**，不是那幾天。想連這個位元都不要，就整個刪掉 `sister.db`
  （她動過手的話，`action-log.jsonl` 是旁邊另一個檔案，要一起刪）
