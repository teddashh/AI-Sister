...
- 識別：四姊妹與 13 位閨密共 17 個 canonical 身分，角色圖全部隨 desktop 離線提供，
  ...
  同意書另有每位四張、共 68 段 bundled Ogg、4,538,740 bytes；逐段逐字稿必須等於 core 當下的
...
17 套角色圖、544 段日常語音與 68 段（4,538,740 bytes）同意書朗讀不依賴我們的伺服器；它們都隨 desktop 安裝。舊 Persona
...
| Persona assets | 17 人本機 catalog + bundled workplace 分層 rig（active-only decode）+ WebP fail-safe（ChatGPT 預設）+ 每人基本 8／擴充 24 的日常語音 + 每人四段同意書朗讀 | 402 張 selected PNG = 35,140,885 bytes；544 段日常 Ogg = 8,920,503 bytes，另有 68 段 consent Ogg；逐檔 pin text/path/bytes/hash/duration/rights/loudness（alpha.137 起每段帶 integrated LUFS 與 true peak，CI 重新解碼對回 manifest；alpha.138 起 CI 另量頭尾空白，每包至少 99% 兩端在 300 ms 以內）；WebView 只從同源 bundled path 播放；recorder/core 保持零網路 |
