...
- [~] `sister query 電話` 能在 < 100ms 撈回三天前畫面上的客服電話，附出處。
  **量過了，一半過一半沒過**（`crates/sister-core/tests/search_latency.rs`，
  45 天語料 = 3,110,400 行字，CI runner）：三個字以上走 trigram 是 0.2 ms、
  整個 token 走 unicode61 是 1.8 ms，都附得出出處。但**兩個字的中文詞沒有
  索引**——trigram 比不了 <3 字，而 unicode61 把「客服專線」整串當成一個
  token，所以只剩全表掃描，224 ms。掃描現在夾在 30 天內（成本不再跟著使用
  時間長大），代價是兩個字的中文查詢碰不到更舊的原文。
  這一條要真的打勾，得先決定要不要加 bigram 索引（原型 0.01 ms，文字索引
  翻倍）。退場條件裡那句 `query 電話` 剛好就是兩個字——它挑到的正是唯一
  沒過的那條路。
