...
- **權利與生成來源隨素材固定**：正式樹只收最終 Ogg、manifest 與 NOTICE；WAV、reference、
  ...
  manifest 逐段帶著量到的值，CI 把每一段重新解碼對回去。alpha.138 起頭尾的空白在整平之前
  先切掉（切點由能量包絡算，Whisper 只做事後驗證：切完台詞的字一個都不准少，證不出來的
  ...
  組響度、真峰值與聲紋檢查。952 段裡 66 段換過；另外 8 段各骰 18 次都沒收下（其中
  6 段三軸仍判有多字），原檔保留。**那 8 段是還沒做完，不是判不了**：144 骰裡有 89 骰
  其實既沒犯這一版自己訂的那條（兩邊都多出字），也沒犯互動短句那包品管自己問的
  那幾題（暴走、沒聲音、長度）；擋掉它們的是我從固定台詞那包搬過來的
  CER ≤ 0.25——而互動短句那支品管的檔頭，正是為了語助詞才不收這條線。要真的
  換掉它們，得先有一條看得見「換成別的字」的判準——那正是那組檢查的檔頭自陳看不到
  的一件事。
