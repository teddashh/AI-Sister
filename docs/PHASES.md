...
- [ ] 磁碟 < 300MB/天（實測數字記入 README）。alpha.47 的 60 秒歸因已回收：
  ...
  main 已讓 WAL 退出每日計帳。按邏輯配置粗算資料庫約 219MB/天，加 250MB 圖
  上限約 469MB/天，仍比預算多約 169MB/天。把圖額度降到 75MB 可粗算到約
  294MB/天，但可點畫面會從約 91 分鐘降到 27 分鐘；這項取捨等 Ted 決定，
  不要求重跑診斷。
...
- [ ] repo public、Apache-2.0、README 首段 = 三張同意書宣言 + 實測足跡數字。
      ...
      main 已排除 WAL 錯帳；粗算仍約 469MB/天，比預算多 169MB/天，真缺口的
      最小修法是把圖額度降到 75MB，等 Ted 決定證據密度取捨。
