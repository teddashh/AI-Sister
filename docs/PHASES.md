...
- 第二張同意有效且大腦已接好時，每個文字問題先交給那支 CLI。CLI 只能要求 1–3 條
  ...
  開頭、中段與結尾，一般問題則取命中時間附近的 L2。另以每條查詢比對卡片 activity，
  最多四張內容命中優先保留，再補時間背景至八張；依時間排序供大腦串起前後文。
  內容命中的卡片原句會先直接顯示，時間背景仍只供成句與出處回查。
...
- [x] 找回率相對 Phase 2 baseline 提升可量測（+facts+session 配置）。
      〔alpha.157：`baseline_text` 和 `facts` 把「昨天下午那份週報寫了什麼」放寬成
      「週報」之後也答得出來，README 第二張表三個配置打平，那張表已經分不出章節多帶了什麼。
      量的工具還在；要再量出差距，得等題庫長到 ≥ 100 題〕
