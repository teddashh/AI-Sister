...
- 🔶 白名單 #1 的**前半段**做完了：`sister watch "<你在等什麼>"`（alpha.71）。
  ...
  - 整個模組在防同一件事：**「還沒發生」有太多冒充者**。她被暫停、她沒在錄、
    她剛開機、她正在讓大腦讀一段、心跳斷了、狀態檔讀不懂——八種「看不到新
    畫面」各有各的句子，每一句都帶著「這不是『還沒發生』」。大腦那一邊也一樣：
    逾時、spawn 失敗、回了讀不懂的字，三種都**不是**「她說還沒」。
  - 結尾三種：等到了／時間到了沒等到／今天的外送預算先用完（後者明說
    「這不是『沒等到』，是我不知道」）。沒有第四種——Ctrl-C 就是把行程殺掉，
    不假裝有優雅收尾。
  ...
  - **還缺**：卡住／跑完是同一個問句問出來的，她分不出「這個 agent 卡住了」
    和「這個 agent 還在想」；沒有通知（要盯著終端機）；沒有「接著推下去」的
    那一半——那是白名單 #1 的後半段，需要手，也需要停止條件。
