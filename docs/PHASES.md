...
- 🔶 `semi-action` 級：平台無關的核心做好了——結構化 grant
  ...
  **alpha.70 有第一個呼叫端了：`sister do`。** 在這之前這 443 行從發表那天起
  ...
  - **還缺**：這一格是**查到什麼記什麼，不是驗證動作成功**——她不會去比對
    「該開的網址真的開了嗎」。而且她不等下一張圖（他人就坐在終端機前面），
    所以「動作之後」那一種在真實使用裡很少見，多數步驟拿到的是「動作之前」。
...
- 🔶 白名單 #1 的**後半段**（「接著推下去」）在 alpha.77 接上了：
  ...
  - **還缺**：離開偵測、交接 offer 那一段還沒做。

**已知還在說謊的地方**（alpha.79 當下，三條）：

1. `sister doctor` 的暫停那一格印
   「**暫停中**（不知道從什麼時候開始）」，而那句話底下是**兩種**成因：
   旗標在、內容壞掉（刪掉它有用），和根本讀不到那條路（刪掉它沒有用）。
   `sister record` 的 Windows 開場那一行 alpha.78 修掉了，doctor 這個呼叫端
   沒有跟著——兩種 `None` 在那一行上仍然長得一模一樣。
2. 「提示只送出去一半就整份丟掉」這條規則守不到一種：一支 CLI **沒讀就
   退場**、而提示剛好整份塞得進作業系統的管子緩衝區。正式上限是 24 KiB，
   Linux 的管子是 64 KiB，所以那份提示算「送完了」，這條規則不會攔它。
   （不是理論——CI run 33249588350 就是這樣紅的，只是那次紅在測試身上。）
3. `sister do` 收尾那一行有六格拒絕，今天真的產得出來的只有兩格
...
（**`sister doctor` 那一格沒有跟著修**，它是另一個呼叫端：`ops.rs` 的
`**暫停中**（{since}）`，兩種成因在那一行上仍然長得一模一樣。這一條在下面
「已知還在說謊的地方」上。這裡原本整段寫的是 `sister doctor`，是我在派工單
上就寫錯了命令名，改的人照著改對了地方、文件卻留著錯的名字——同一段話裡
第三行自己就寫著呼叫端是 `windows_record`。）
