...
| 磁碟 | **3.2 GB/天** | ⚠ 只是 60 秒外推，待歸因；< 300 MB/天仍未通過 |
...
磁碟同場實際長了 5.4MB：畫面 3.3MB、其他 2.1MB。3.2GB/天是短時間外推，
不是整天實測；真正的下一個 milestone 是歸因「其他」並驗整天數字。alpha.47
只加診斷：`record` 收尾會分列 SQLite 邏輯配置與具名物件、main／WAL／SHM
實體檔案、畫面帳差與資料目錄旁檔。**先等真 Windows 輸出，不要從機制猜答案。**
WAL 是可重用工作檔，短場檔長淨變化不准直接換算成每天永久增長；診斷本身也排在
CPU／RAM／牆上時間凍結之後，不會把自己的掃描成本混進原量測。
...
- **擷取那條路已經結案，繼續不要動**：`sister bench` 量到建立 GDI 物件 0.0–0.1 ms
  ...
  alpha.46 的主成本，**不要做**。
- **磁碟「其他」的成因目前仍沒查出來。已排除的錯誤診斷不要再推一次：**
