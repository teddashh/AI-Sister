...
資料庫約 **219MB/天**；加上 **250MB/天**圖上限約 **469MB/天**，仍比預算多約
**169MB/天**。沒有一個已證明安全的小 schema 修法補得滿；最小能過的是把預設
圖額度降到 75MB（粗算約 294MB/天），但會讓這場速度下可點開的畫面從約 91 分鐘
降到 27 分鐘。**這是負責人取捨，要停下來問 Ted。** 不要從 4.3GB/天去猜機制，
也不要再發一版只為重跑相同診斷。
