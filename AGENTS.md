...
9. **`check-pet-says-why.mjs` 裡，取樣的計時器和被取樣的計時器是兩支。**
   畫面上那個秒數是產品的 `setInterval(paintThinking, 1000)` 重畫出來的，
   **要連跳四次**才跨過四秒線；而測試用 `await tick(N)` 取樣，那是**只跳一次**的
   `setTimeout`。兩支排在同一個 event loop 上，機器一忙，跳四次的那支落後得多，
   取樣就讀到還沒重畫的舊字。餘裕算得出來：⑪ 那組上界 800 ms，上面那組只有 360 ms。
   2026-09-23 這個 race 讓**四輪派工白跑**，一族三條都中過。
   **不要用 `tick(N)` 去猜「它應該重畫幾次了」**——用 `awaitRepaint(p, 現況)` 等它
   真的變，期望值也不要寫死，拿測試自己的碼錶算（理由寫在那支函式上面）。
   同族的判準：**測試裡只要出現「等 N 毫秒然後斷言一個會隨時間變的值」，
   就先問那個值是誰重畫的、那支計時器要跳幾次。**

10. **一個旗標的不變式，要用原始碼結構守，不要只用情境守。**
    alpha.151 的 `showingProvisional` 要「為真當且僅當答案區裡只有那句承諾」。
    三個會替換答案區內容的函式裡，第三個（`showConsentCompletion`）沒有清掉它，
    而**拿掉那一行是 880 綠 0 紅**——因為那條路需要「同意書不是被問題打開的」，
    那個情境裡根本沒有 in-flight 的 `ask` 可以讓它失敗。**情境測不到。**
    會落到這一格的判準是：不變式講的是「**所有**寫入端都要做 X」，
    而其中某些寫入端走得到的情境，產品自己生不出來。
    改成數寫入端（`check-pet-says-why.mjs` 已有 `functionRanges()` 可以用）：
    掃出所有呼叫那個寫入動作的函式名，斷言每一支都做了 X、斷言設真的只有一處，
    **再加一條「真的抽到至少 N 個」的前提斷言**——否則掃描器空手的時候，
    前兩條會空轉著叫綠，而「掃到 0 個、每一個都合格」印出來和真的通過一模一樣。
