...
//! ```
//!
//! （第一行來自 `timeline.js` 的 `guessRow`：`place === "current"` 走的是自己
//! 那一臂，印時間、不印信心。「她猜的（模型說的信心 …）」是同一串 `if` 最後
//! 那個 `else`，只有日列表那些卡到得了——我原本在這裡把那一行當成「這一刻」
//! 那張卡的樣本，那是假的。壞掉的是第三行：印「原版：」的那個 `if` 在整串
//! `place` 判斷**結束之後**，兩種卡都會印。這裡一樣不寫行號，理由同下。）
//!
