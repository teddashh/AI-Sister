...
pub fn prepare(
    ...
    let header = format!(
        concat!(
            ...
            "每一筆來源都帶 kind。kind=\"reading\" 是你稍早依紀錄想過一輪的可推翻結論；ocr 是畫面文字辨識，assistive 是輔助介面回報的前景可見文字，clipboard 是剪貼簿文字，window_title 是視窗標題，url 是記下的網址，told 是使用者親口告訴你的話，不是你觀察到的內容。其他來源照 kind 理解，unknown 代表來源不明，不可猜成 OCR。\n",
