...
impl DbStats {
    ...
    ///
    /// **「她記的」不含 `queries`，而這條線是整支函式最重要的東西。** 上一版
    /// 叫 `nothing_left`，把他自己打進搜尋框的字也算進去，理由聽起來很對：
    /// 那也是一列，說「一列都不剩」就是假的。但 `queries` 會在清空**之後**
    /// 長出來——`forget` 完接著問一句「真的沒了嗎」，就有一列。於是這個述詞
    /// 翻成 false，`Emptiness` 掉到 `Fresh`，`stats` 的 ⚠ 整段消失，
    /// `doctor` 說「（還沒有任何內容）」，`facts` 說「她還沒錄過」——他一秒
    /// 前才親手刪掉的一整天，被他自己的下一個指令蓋掉了。
    ///
    /// 那句「一列都不剩」的假話不是靠這裡修的，是靠呼叫端把話講準：它現在說
    /// 的是**她記下來的東西**沒了，不是這顆檔案是空的。
    ///
    ...
    pub fn nothing_recorded_left(&self) -> bool {
        let Self {
            ...
            system_events,
            sessions,
            // **不算**，理由見上面第二段和 `queries` 欄位自己的註解：這是他打
            // 的字，不是她記的東西，而且它會在刪除之後才出現。
            queries: _,
            // 底下這些不是「有幾件事發生過」：
