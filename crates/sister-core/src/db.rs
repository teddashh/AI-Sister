...
impl Db {
    ...
    ///
    /// 問句詞和口語開頭都不在這裡認。只問索引會停在她看過的「怎麼」「所以」
    /// 前面——那是 `retry_candidate` 先切掉的理由。問句詞一張、口語開頭另一張，
    /// 都只活在那一次放寬裡。這一支仍然沒有雜訊詞表：開頭連續零命中就跳，
    /// 但不從類型詞起頭。「開頭只問索引」說的是這一件事。
    ///
    ...
    /// 的條件不進那一圈，它仍是必要條件，FTS 的 AND 也還在。跨在「的」「了」
    /// 上的雙字（「報的」「的連」）這裡照樣是條件；再下一步 `joint_retry` 在
    /// 那裡切開，兩個字以上的段不管看過沒有都留著。
    ///
    ...
    ///
    /// 類型詞只認 `QUERY_KIND_TABLE` 和 `WHEN_ASKS` 那兩張。尾巴碰到類型詞就停、
    /// 把那個詞留著；「電話」常常沒寫在畫面上，放掉它 facts 就失去種類。「什麼時候」
    /// 在這裡也會留著，切掉它的是呼叫端 `retry_candidate`，不是這一支。英文沿用
    /// `query_kind_word_hits` 的邊界：完整 token，可接複數 s，hotel 裡的 tel
    /// 不是類型詞。
    ///
    ...
    pub(crate) fn indexed_candidate(&self, query: &str) -> Result<IndexedCandidate> {
        ...
        let Some(first) = first else {
            ...
            // 會退成任意一個網址。這一半只問索引，問句詞和口語開頭都不在這裡。
            return Ok(IndexedCandidate::NoneSeen);
        };
    ...
    /// 她自己寫的那句判讀，照**內容**找得到。
    ///
    /// 三支 `readings_*` 全是時間窗：一張卡只有在它附近的 L0 原文或 fact 先命中
    /// 的時候才進得了答案。所以「這段在處理退款」這句話，只要「退款」兩個字
    /// 沒真的出現在螢幕上，以前就永遠搜不到。這一支補的是那個洞。
    ///
    /// 索引只有 `activity`。entities／commitments 是 JSON 陣列，丟進去會讓 `[`
    /// 和 `"` 變成可搜尋的垃圾；entity 在 L3 有自己的表，那是另一個題目。
    ///
    /// **三個字以上走 trigram MATCH，兩個字以下在同一張表的內容上 LIKE。**
    /// trigram 對兩個字的中文是 0 命中（`reading_search_tokenizer_measurement`
    /// 實測：`退款` → 0、`退款申請` → 1），所以短詞非掃不可。
    ///
    /// 那個掃描的代價量過了，**開發機、release build、記憶體資料庫，取五次最小值**：
    ///
    /// | 卡片數 | 短詞（LIKE 全掃） | 長詞（MATCH） |
    /// |---:|---:|---:|
    /// | 1,000 | 142 µs | 41 µs |
    /// | 10,000 | 820 µs | 24 µs |
    /// | 50,000 | 4.1 ms | 26 µs |
    ///
    /// 線性，約 82 ns 一張卡。對照的是一整題裡 CLI 那兩趟的 21–120 **秒**，
    /// 所以這個掃描不是瓶頸，不值得為它改成 bigram 那種「每次寫入都要在 Rust
    /// 裡切詞」的形狀。**這是開發機的數字，不要把它變成 CI 門檻**——貼著本機
