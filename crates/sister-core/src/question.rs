...
const FILLER_CJK: &[&str] = &[
    "發生", "什麼", "甚麼", "事情", "幹嘛", "幹麼", "一下", "剛好", "我", "你", "妳", "她", "他",
    "在", "做", "了", "過", "的", "是", "有", "嗎", "呢", "吧", "啊", "喔", "事", "看", "到", "些",
    "那", "這", "耶", "呀", "個",
];
...
mod tests {
    ...
    /// 問她的時候常寫「妳」。和「你」一樣是虛字：剝完剩的一樣，問剛剛的也一樣。
    #[test]
    fn ni_written_with_the_female_radical_is_filler_too() {
        assert_eq!(terms("妳有看到月報連結嗎"), "月報連結");
        assert_eq!(terms("你有看到月報連結嗎"), "月報連結");
        assert_eq!(shape("妳剛剛在做什麼"), Shape::Recent);
        assert_eq!(shape("你剛剛在做什麼"), Shape::Recent);
    }
