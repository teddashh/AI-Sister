//! 她自己佔了多少：CPU、記憶體、磁碟。
//!
//! Phase 0 要量 CPU、RAM、磁碟，而在這個模組存在之前，**沒有任何辦法知道
//! 它們是多少**。一個量不到的數字不是基準，也不是預算。2026-08-23 起 CPU
//! 仍然每場照實量，但不再是 Phase 0 blocker；alpha.46 的真機活躍寫程式基準
//! 是 44.0%。RAM < 400MB 與磁碟 < 300MB/天仍是這一階段的驗收條件。
//!
