...
只拿 SQLite 邏輯配置與畫面外推。按 156KB 粗算，資料庫約 219MB/天；再加每日
250MB 圖上限約 469MB/天，離 `<300MB/天` 仍差約 169MB/天。最小能補完的改動
是把圖額度降到 75MB（粗算約 294MB/天），代價是這場速度下可點開的畫面從約
91 分鐘降到 27 分鐘；文字與搜尋不減。這是證據密度取捨，要由 Ted 決定，
不是再跑一次診斷能回答的事。
