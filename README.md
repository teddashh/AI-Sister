...
- `local-recording`：「我同意在我的硬碟上記錄我的螢幕。」沒有這一張，`sister record`
  ...
  簽了才准在本機記錄。
- `cloud-reading`：「我同意把去識別化後的文字（OCR 抽出來的字，永不含畫面）交給我在設定裡指定的本機 CLI，由那支程式去做解讀。」
  沒有這一張，解釋層一次都不會呼叫那支 CLI。畫面永不離開這台機器；出去的只有去敏後的字，而且只交給你自己設定、已經在跑的那支命令。
...
| `facts_session` | 4/4（100.0%） | 5/5（100.0%） | 4/4（100.0%） | 0 | US$0/天 |
...
| 配置 | 找回率@5 | 答案正確率 | 出處正確率 | 模型呼叫 | 成本 |
|---|---:|---:|---:|---:|---:|
| `baseline_text` | 1/2（50.0%） | 2/3（66.7%） | 1/2（50.0%） | 0 | US$0/天 |
| `facts` | 1/2（50.0%） | 2/3（66.7%） | 1/2（50.0%） | 0 | US$0/天 |
| `facts_session` | 2/2（100.0%） | 3/3（100.0%） | 2/2（100.0%） | 0 | US$0/天 |
