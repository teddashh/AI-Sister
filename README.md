...
同樣是 GitHub Actions 的 runner、release build，每次 push 都會重跑）：

| 你打了什麼 | 走哪條路 | 實測 |
|---|---|---|
| `客服專線`（三個字以上） | trigram 索引 | 0.2 ms |
| `0800`（整個 token） | unicode61 索引 | 1.8 ms |
| `客服`（兩個字的中文） | **沒有索引，只能掃** | 224 ms |
| 查不到的東西 | **沒有索引，只能掃** | 213 ms |

最後兩列是一個真的缺口，不是還沒調校：trigram 比不了少於 3 個字，而
unicode61 把「客服專線」整串當成**一個** token（不是逐字切），所以
`MATCH "客服"` 是 0 筆——而兩個字正是中文裡最常見的詞長。
剩下唯一找得到的辦法是掃過所有文字，成本跟你用了多久成正比。
現在掃描被夾在 30 天內——所以那 224 ms 不會再長大，但也不會變小。
代價是**兩個字的中文查詢找不回 30 天以前的東西**；`sister doctor` 會直接
告訴你，你自己的資料庫裡有多少行字已經在那條線外面。
真正的解是補一個 bigram 索引（原型量到 0.01 ms），代價是文字索引大約翻倍。

（順帶一提，同一份 benchmark 在開發機上是 106 ms，在 runner 上是 224 ms
——2.1 倍。所以上面那張表裡任何一個貼著門檻的數字，換一台機器就會翻面。
這也是為什麼「掃描不會跟著資料長大」那件事是用行為測的，不是用碼錶測的。）
