...
章節在那種問法上什麼都不會多帶來，這一列照實登出來。下面這份是另一份
fixture（一個 115 分鐘的合成下午、3 題），專放帶時間範圍的問法，由同一支腳本
以同樣方式生成與鎖定：
...
| 配置 | 找回率@5 | 答案正確率 | 出處正確率 | 模型呼叫 | 成本 |
|---|---:|---:|---:|---:|---:|
| `baseline_text` | 2/2（100.0%） | 3/3（100.0%） | 2/2（100.0%） | 沒跑腦 | 沒跑腦 |
| `facts` | 2/2（100.0%） | 3/3（100.0%） | 2/2（100.0%） | 沒跑腦 | 沒跑腦 |
| `facts_session` | 2/2（100.0%） | 3/3（100.0%） | 2/2（100.0%） | 沒跑腦 | 沒跑腦 |
<!-- END GENERATED: recall-session-benchmark -->

**這份現在三個配置打平，所以它分不出章節多帶了什麼。**「我昨天下午在弄什麼」
只問時間，三個配置都直接列出那段時間裡的東西。「昨天下午那份週報寫了什麼」
在 alpha.156 只有 `facts_session` 答得出來：另外兩個先拿「份週報寫」去比對，
再放掉開頭的「份」改用「週報寫」，螢幕上都沒有。alpha.157 起尾巴對不到的「寫」
也會放掉、改用「週報」去找，於是打平。分母本來就只有 2 題，
不是有統計份量的數字；要有份量得等題庫長到 ≥ 100 題（Phase 2 的退場條件之一，
還沒到）。
