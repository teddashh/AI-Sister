...
function renderHits(
  ...
  if (hits.length === 0 && facts.length === 0 && !hasChapters && matchedReadings.length === 0) {
    ...
    if (provisional) {
      ...
      if (canRememberTold && consentGuideSheet === null && !earlyPass) {
        empty.textContent += "我記憶中都沒有這一塊，你可以告訴我嗎?";
        awaitingTold = true;
        setConsentGuideInput(true);
      }
      hitList.append(empty);
  ...
  // **這裡不檢查「他剛剛收起過」，而失敗那一條會檢查——不對稱是故意的。**
  //
  // 失敗那一條（`ask` 的 catch）尊重收起，因為它要交代的只有「這題沒答成」，
  // 而那句話在角落的 notice 行上也講了一次，收著不看不會少掉什麼。
  // 這一條不一樣：全檔把答案區打開的只有 `showAnswerHits` 一個寫入端，
  // 而且沒有任何一顆按鈕能重新打開收起的氣泡。成功也尊重收起的話，
  // 他等了二三十秒的那份答案就**再也看不到了**，只能整句重打再等一次。
  //
  // 看到兩條分支不一樣而想順手「修」成一致之前，先去看
  // `check-pet-says-why.mjs` 裡「打開答案區的寫入端只有一處」那條斷言。
  showAnswerHits();
  paintConversation();
...
async function ask(event = null) {
  ...
  try {
    ...
    // **先開口，再想。**
    //
    // 他按下 Enter 看到的是一片空白加一個跳動的秒數，因為本機檢索被排在
    // **規劃**那一趟 CLI 後面（他上傳的診斷上量到 21.2–30.7 秒）。成句那一趟
    // ——最久到 120 秒 timeout——本來就排在本機檢索**後面**，擋住她開口的不是它。
    // 這一段把她自己記得的東西先畫出來，CLI 回來之後底下那一趟再整份重畫。
    //
    // **這一趟有多快，只講量得到的那幾步。** 住在 `sister-core` 的部分在 30 天
    // 語料（172,800 張畫面 = 2,073,600 行字、release、開發機）上是：零命中的
    // 檢索 0.13 ms、章節重算走唯讀那條 2.57 ms（正式那條會寫回 segment 表，
    // 36.58 ms）、判讀撈卡 0.03 ms。Tauri command 的進出、序列化與 IPC 在這台
    // 機器上量不到（`apps/desktop` 缺 dbus-1 編不起來），所以這幾個數字是
    // **下界**，不是全程。真數字要看題庫的 `latency_ms`，那是真的提問量出來的。
    //
    // `docs/PHASES.md` 那句「四條路都在 1 ms 以內」講的是 `db.search_during`
    // 那四條**檢索**路，不是這一整趟——不要拿它來替整條路背書。
    //
...
//
// **alpha.153 起它們預設收起來了，要先按那顆「為什麼？」。** 那顆按鈕是
// 這條 demo 路上唯一需要動手的東西：不按，每一個 `&blind=` 看起來都一樣。
// （收起來的理由和代價寫在 `styles.css` 的 `.hits-why-disclosure` 上面。）
//
